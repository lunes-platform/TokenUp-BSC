...
describe("Up", function () {
  before(async function () {
    this.Up = await ethers.getContractFactory("Up");
  });

  beforeEach(async function () {
    this.upToken = await this.Up.deploy();
    await this.upToken.deployed();

    this.decimals = await this.upToken.decimals();
    ...
    this.signerContract = this.upToken.connect(signers[1]);
  });

  // Test cases
  it("Creates a token with a name", async function () {
    expect(await this.upToken.name()).to.exist;
    // expect(await this.upToken.name()).to.equal('Up');
  });

  it("Creates a token with a symbol", async function () {
    expect(await this.upToken.symbol()).to.exist;
    // expect(await this.upToken.symbol()).to.equal('FUN');
  });

  it("Has a valid decimal", async function () {
    expect((await this.upToken.decimals()).toString()).to.equal("18");
  });

  it("Has a valid total supply", async function () {
    const expectedSupply = ethers.utils.parseUnits("1000000000", this.decimals);
    expect((await this.upToken.totalSupply()).toString()).to.equal(
      expectedSupply
    );
  });

  it("Is able to query account balances", async function () {
    const ownerBalance = await this.upToken.balanceOf(this.ownerAddress);
    expect(await this.upToken.balanceOf(this.ownerAddress)).to.equal(
      ownerBalance
    );
  });
  ...
  it("Transfers the right amount of tokens to/from an account", async function () {
    ...
    await expect(
      this.upToken.transfer(this.recipientAddress, transferAmount)
  ...
  it("Allows for allowance approvals and queries", async function () {
    ...
    expect(
      await this.upToken.allowance(this.recipientAddress, this.ownerAddress)
    ).to.equal(
      ethers.utils.parseUnits(approveAmount.toString(), this.decimals)
    );
  });
  ...
  it("Allows allowance to be increased and queried", async function () {
    ...
    const previousAllowance = await this.upToken.allowance(
      this.recipientAddress,
      this.ownerAddress
    );
